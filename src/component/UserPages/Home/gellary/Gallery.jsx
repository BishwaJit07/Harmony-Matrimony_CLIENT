  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
      <div className="col-span-2 flex flex-col gap-3">
        <PhotoContainer isHome={isHome} imgStyle='h-[363px] w-full object-cover' img={img1} />
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
          <PhotoContainer isHome={isHome} img={img2} />
          <PhotoContainer isHome={isHome} img={img3} />

        </div>
      </div>
      <div className="flex flex-col gap-3">
        <PhotoContainer isHome={isHome} imgStyle='h-[282px] object-cover w-full' img={img4} />
        <PhotoContainer isHome={isHome} imgStyle='h-[282px] object-cover w-full' img={img5} />
      </div>
    </div>
  );
}

export const PhotoContainer = ({ img, imgStyle, isHome }) => {
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden group">
      ...
      <div className="absolute flex justify-between bg-slate-800 w-full  bg-opacity-50 group-hover:bottom-0 -bottom-24 px-5 py-4 duration-300">
        ...
        {isHome && <Link to='/galleryPage' className='text-white border border-white px-10 py-4 rounded-full text-[18px] font-lato '>View More</Link>}
      </div>
    </div>
  )
}